...
aws eks wait cluster-active --name $tmc_development_cluster

aws eks create-nodegroup \
        --cluster-name $tmc_development_cluster \
        --nodegroup-name "${tmc_development_cluster}-node-group" \
        --disk-size 500 \
...
aws eks update-kubeconfig --name $tmc_development_cluster --region $aws_region_code

kubectl config rename-context ${arn}/${tmc_development_cluster} $tmc_development_cluster
