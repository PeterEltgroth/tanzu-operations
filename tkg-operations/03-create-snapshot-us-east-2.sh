...
aws s3 cp s3://tkg-operations/operator-instance.txt ./
...
aws ec2 create-snapshot --volume-id $volume_id --tag-specifications 'ResourceType=snapshot,Tags=[{Key=Name,Value=tkg-operator-snapshot-us-east-2}]'
