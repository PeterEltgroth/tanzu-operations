...
bootstrapper_url=$(cat tanzu-bootstrapper-files/${operator_prefix}-bootstrapper)

export AWS_REGION=$aws_region_code

if [[ $aws_region_code = "us-east-2" ]]
then
    instance_id=$(aws ec2 run-instances --image-id ami-0fb653ca2d3203ac1 --instance-type t3a.2xlarge --block-device-mappings '{"DeviceName": "/dev/sda1", "Ebs": {"DeleteOnTermination": true, "VolumeSize": 500}}' --tag-specifications "${key_name}" --security-group-ids tanzu-operations-${aws_region_code} --key-name tanzu-operations-${aws_region_code} --user-data "${bootstrapper_url}" --output text --query "Instances[0].InstanceId")
